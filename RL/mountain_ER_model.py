...
for i in range(EPOCH):
    ...
    if i % 200 == 0:
        str_i = str(i)
        main.model.save_weights('new_param/model_'+str_i+'_epi_24_24_24_fix.h5')
...
for r in reward_list:
    if count == 50:
        sum_reward_list.append(sum_value)
        count = 0
        sum_value = 0
    sum_value += r
    count += 1

x_len = len(sum_reward_list)
x = np.arange(0, x_len, 50)
x = np.array(x)
y = np.array(sum_reward_list)

plt.plot(x,y)
plt.savefig('MountainCar_ave.png')
plt.close()
